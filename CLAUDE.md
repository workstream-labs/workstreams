...
The CLI is invoked as `ws`. Key subcommands: `init`, `create`, `run`, `list`, `diff`, `destroy`, `merge`, `resume`, `dashboard`, `view`.
...
- `dashboard.ts` — `ws dashboard`: opens the interactive TUI dashboard. IDE-style layout with workstream list, inline logs/diff viewer, chat input for prompting agents, and review comments. Also exports editor helpers used by `view.ts`.
- `view.ts` — `ws view <name>`: opens a workstream in your editor. Supports `-e` for a specific editor and `--no-editor` to just print the worktree path. Auto-detects and persists the user's preferred editor in state.
