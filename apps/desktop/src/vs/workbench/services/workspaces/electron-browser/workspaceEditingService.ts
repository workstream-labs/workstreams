...
export class NativeWorkspaceEditingService extends AbstractWorkspaceEditingService {
	...
	private async saveUntitledBeforeShutdown(reason: ShutdownReason): Promise<boolean> {
		...
		/**
		 * When the orchestrator is managing the workspace, silently discard
		 * the untitled workspace without prompting. The orchestrator persists
		 * its own state (repos, worktrees) in application-scoped storage.
		 */
		if (this.storageService.get('orchestrator.repositoryState', StorageScope.APPLICATION)) {
